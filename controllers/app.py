"""
Ryu 主控制器 — 纯透传架构 + 拓扑发现 + 性能检测 + 路由下发 + 流量分类 (Phase 5)
...
数据流（全量透传，主控不做 EtherType 分类）：
  PacketIn → SecurityFilter(前置) → Ring Buffer → Dispatcher → Workers
    ...
    └─ IP  (0x0800/0x86DD) → west_queue → ArpHandler (首包路由触发 + 流表下发)
                            └─ classification_queue → FlowTracker → Classifier (Phase 5)
...
下游消费：
  ...
  classification_queue → FlowTracker → Classifier (五元组聚合 + 朴素贝叶斯五分类, Phase 5)

多线程架构：
  - Dispatcher 线程 × 1: pop(RingBuffer) → hash → put(Worker.input_queue)
  - Worker 线程 × 3: get(input_queue) → 过滤/按 EtherType 分类 → put(topo_east + perf_east + west + classification)
  ...
  - FlowTracker 线程 × 1: get(classification) → 五元组聚合 → Classifier (Phase 5)
  - Trainer 线程 × 1: 每 5 分钟从 MySQL 拉数据 → Classifier.fit() (Phase 5)
...
from modules.classification.flow_tracker import FlowTracker, FlowFeatures
from modules.classification.classifier import NaiveBayesClassifier
...
class SDNController(app_manager.RyuApp):
    ...
    def __init__(self, *args: Any, **kwargs: Any):
        ...
        self._shared_classification_queue: "queue.Queue[Any]" = queue.Queue(maxsize=10000)  # Phase 5
        for worker in self.dispatcher.workers:
            worker.set_output_queues(
                ...
                classification=self._shared_classification_queue,  # Phase 5
            )
        ...
        # Phase 5: 流量分类日志异步写入（使用 insert_class_log_batch，独立队列）
        self.class_log_writer = MySQLWriterThread(
            mysql_client=self.mysql_client,
            write_fn=self.mysql_client.insert_class_log_batch,
            flush_interval=1.0,
            batch_size=200,
            queue_maxsize=10000,
            logger=self.logger,
        )
        self.class_log_writer.start()

        self.logger.info("🗄️  Phase 3: MySQL connected (async writers: topology + perf + class_log)")
        ...
        self.classifier = NaiveBayesClassifier(logger=self.logger)

        # FlowTracker: 从 classification_queue 消费 IP 包，按五元组聚合流特征
        self.flow_tracker = FlowTracker(
            classification_queue=self._shared_classification_queue,
            logger=self.logger,
        )

        # 流输出回调: FlowTracker._output_flow() → Classifier.predict() → Redis + MySQL
        def _on_flow_flush(feats_list: List[FlowFeatures]) -> None:
            for feats in feats_list:
                try:
                    result = self.classifier.predict(feats)
                    flow_key = result["flow_key"]

                    # 写入 Redis class:result:{flow_key}
                    redis_data = {
                        "type": result["type"],
                        "priority": result["priority"],
                        "confidence": str(result["confidence"]),
                        "is_pseudo": "1" if result["is_pseudo"] else "0",
                    }
                    try:
                        self.redis_client.client.hset(
                            f"class:result:{flow_key}", mapping=redis_data,
                        )
                    except Exception:
                        if self.logger:
                            self.logger.debug(
                                "[Classifier] Redis write failed for %s", flow_key,
                            )

                    # 异步写入 MySQL flow_class_log
                    self.class_log_writer.enqueue({
                        "flow_key": flow_key,
                        "src_ip": feats.src_ip,
                        "dst_ip": feats.dst_ip,
                        ...
                        "avg_packet_size": feats.avg_packet_size,
                        "avg_iat": feats.avg_iat,
                        "packet_count": feats.packet_count,
                        "predicted_class": result["type"],
                        "confidence": result["confidence"],
                        "is_pseudo_label": 1 if result["is_pseudo"] else 0,
                    })

                    if self.logger:
                        self.logger.debug(
                            "[Classifier] %s → %s (conf=%.3f, pseudo=%s)",
        ...
        self.flow_tracker.set_on_flow_flush(_on_flow_flush)
        self.flow_tracker.start()
        self.logger.info("🔬 Phase 5: FlowTracker started (classification_queue → Classifier)")

        # Trainer: 每 5 分钟从 MySQL flow_class_log 拉数据训练
        ...
        self.trainer.start()
        self.logger.info("🧠 Phase 5: Trainer started (interval=300s, window=1h)")
        ...
        self.logger.info("=" * 60)
        self.logger.info("SDN 主控制器启动（Phase 5: 流量分类）")
        ...
        self.logger.info("📍 分类通道: PacketIn(IP) → RingBuffer → Dispatcher → Workers → classification_queue → FlowTracker → Classifier (Phase 5)")
        ...
        self.logger.info("📍 存储: Redis (快照+version+class:result) + MySQL (changelog + perf + class_log 异步批量写入)")
    ...
    def _log_comprehensive_stats(self):
        ...
        # Phase 5: 流量分类统计
        tracker_stats = self.flow_tracker.stats()
        classifier_stats = self.classifier.stats()
        trainer_stats = self.trainer.stats()
        class_log_writer_stats = self.class_log_writer.stats()
        self.logger.info(
            f"📊 [Phase 5] Classification: ips={tracker_stats['total_ips']} "
            f"flows_output={tracker_stats['total_flows_output']} "
            f"active={tracker_stats['active_flows']} | "
            f"clf(pred={classifier_stats['total_predictions']} "
            f"pseudo={classifier_stats['pseudo_ratio']:.1%} "
            f"trained={classifier_stats['is_trained']}) | "
            f"trainer(fits={trainer_stats['total_trainings']} "
            f"samples={trainer_stats['total_samples_pulled']}) | "
            f"class_log(enq={class_log_writer_stats['enqueued']} "
            f"wr={class_log_writer_stats['written']} "
            f"q={class_log_writer_stats['queued']})"
        )
